...
function createWorld(currentDeploymentId: string) {
  ...
  const world = {
    // Present so the guard treats this as a world with atomic deployments
    // (world-vercel). Absent worlds (local/postgres) skip the guard entirely.
    resolveLatestDeploymentId: vi.fn().mockResolvedValue(currentDeploymentId),
...
describe('failRunIfDeploymentMismatch', () => {
  ...
  it('skips worlds without atomic deployments (no resolveLatestDeploymentId)', async () => {
    // Local/self-hosted worlds report a synthetic, version-tagged deployment
    // id that varies within one environment; a "mismatch" there is not real.
    const { world, eventsCreate } = createWorld('dpl_current');
    // biome-ignore lint/performance/noDelete: drop the capability for the test
    delete (world as { resolveLatestDeploymentId?: unknown })
