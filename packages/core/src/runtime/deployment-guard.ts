...
export async function failRunIfDeploymentMismatch({
  ...
  // Deployment affinity is only meaningful in worlds with atomic, immutable
  // deployments — world-vercel, the one world that implements
  // `resolveLatestDeploymentId` (see its interface doc). Local and self-hosted
  // worlds report a synthetic deployment id (e.g. `dpl_local@<sdk-version>`)
  // that legitimately differs across SDK versions within a single logical
  // environment, so a "mismatch" there is not a real cross-deployment
  // delivery — guarding would fail ordinary local runs after a version bump.
  if (typeof world.resolveLatestDeploymentId !== 'function') {
    return false;
  }
